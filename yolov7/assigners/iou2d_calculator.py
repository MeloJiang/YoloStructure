...
def cast_tensor_type(x, scale=1, dtype=None):
    if dtype == 'fp16':
        # scale is preventing overflows
        x = (x / scale).half()
    return x


def fp16_clamp(x, min_value=None, max_value=None):
...
def iou2d_calculator(bboxes1, bboxes2, mode='iou',
                     is_aligned=False, scale=1, dtype=None):
...
def bbox_overlaps(bboxes1, bboxes2, mode='iou', is_aligned=False, eps=1e-6):
    ...
    if mode in ['iou', 'iof']:
        return ious
    # calculate gious
    enclose_wh = fp16_clamp(enclosed_rb - enclosed_lt, min_value=0)
